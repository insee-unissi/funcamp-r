...
donnees_secretes <- read_excel("../../data/nbguerriers2.xlsx")
```

## Vérifions tes connaissances avant de continuer

Quel est le code pour faire un nuage de point (sortilège `gf_point()`) à partir de la table *base_individus* pour représenter le poids (variable `poids`) en fonction (signe `~`) de la taille (variable `taille`) ?
...
Bravo, tu maîtrises parfaitement les fondamentaux des graphiques ! Mais, entre nous, le résultat n'est pas très esthétique pour l'instant. Et si nous apprenions, avec le langage runique, à produire des graphiques chatoyants ?
...
L'ingrédient **color** sera donc égal à ~ sexe.
...
Au risque de rendre le graphique très difficile à lire, tu peux aussi changer la forme des points du nuage de points en changeant la valeur de l'ingrédient **shape** (forme en anglais). Soit en lui affectant une valeur fixe shape = 1 (un chiffre entre 0 et 25) par exemple ou en lui affectant une valeur qui varie comme cela shape = ~ variable.
