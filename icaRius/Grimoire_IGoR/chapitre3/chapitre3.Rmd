...
Pour avancer dans son aventure, Icarius dispose de son précieux grimoire, qu'il a pu alimenter au fur et à mesure de ses rencontres avec des mages dans les villages précédents. Ce grimoire renferme une multitude de sortilèges - appelés **fonctions** dans le langage des Runes- permettant chacun d'accomplir une tâche bien spécifique. Nous avons, dans le chapitre précédent, fait la découverte d'un premier sortilège, `import()`, pour charger des données dans R. 

Ces sortilèges seront d'un grand secours pour mener à bien les différentes missions qui attendent icaRius durant son périple... Nous allons, dans ce chapitre, un découvrir un peu plus sur les sortilèges, et tant que nous y sommes, produire des statistiques grâce à cette magie !
...
Il permet de visualiser en sortie (**output**) des statistiques fondamentales (moyenne, médiane...) sur l'ensemble des variables d'un fichier de données. Mais également le nombre de valeurs manquantes, le nombre de valeur unique... et même un mini histogramme qui symbolise la distribution de la variable.  
Il a d'autres avantages encore, que vous découvrirez sans doute un jour dans votre quête...

Essayons sur un fichier factice appelé `mon_fichier`, qui se compose de deux variables `ma_variable_1` et `ma_variable_2`. 
...
- la médiane (*p100*), qui détermine le seuil séparant les valeurs en deux parties égales, une moitié des valeurs qui sont toutes inférieures ou égales à la médiane et l'autre moitié des valeurs, qui sont toutes supérieures ou égales à la médiane
- le premier quartile (*p25*) et le troisième quartile (*p75*). Le premier quartile est le seuil pour lequel 25% des valeurs de la variable sont inférieures ou égales à ce seuil. Le troisième quartile est le seuil pour lequel 25% des valeurs de la variable sont supérieures ou égales à ce seuil.  
...
Le livre de récolte du fermier Galia est tenu tout comme celui du fermier Tellus : mêmes variables, même nombre d'enregistrements.  
Puisque nous nous intéressons précisément à la production moyenne en litres de filtre de Mandragore, sélectionnons de suite la variable `Litres` en lui appliquant le sortilège `skim()`
