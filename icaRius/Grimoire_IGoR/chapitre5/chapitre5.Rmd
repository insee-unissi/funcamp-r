...
livre_ambroisie$ivresse = livre_ambroisie$puissance^(4)/100
...
Essayons un autre type de graphique, en traçant la courbe qui lie l'ivresse de Batreb à la puissance de la liqueur goûtée. Le sortilège `gf_line()` sera utilisé, en croisant la variable ivresse et la variable puissance. 


```{r t2, collapse = TRUE}
# Représenter la courbe présentant le lien 
# entre l'ivresse de Batreb et la puissance de la liqueur
livre_ambroisie %>% gf_line(ivresse ~ puissance)

```

Hic ! Visiblement, le bon Batreb connaît une croissance exponentielle de son ivresse quand la liqueur s'intensifie !
...
```{r t3, collapse = TRUE}
# Représenter sous forme de barre
...
# en ajoutant un titre et une couleur
livre_ambroisie %>% 
  gf_line(ivresse ~ puissance, title = "Boire un petit coup c'est agréable...", color = "tomato")
