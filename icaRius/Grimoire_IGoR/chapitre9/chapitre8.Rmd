...
naissances <- read_csv2(file="E:/icaRius/data/naissances.csv")
```
...
A vous de prendre la plume !
```{r module5-question2,exercise=TRUE}
# Filtrer le registre des naissances pour l'année 1212 
# et enregistrer le résultat dans l'objet naissances_1212


# Ajouter la variable Ratio comme le ratio
# entre la variable Taille et la variable Poids au carré


# Trier le fichier naissances_1212 selon le Ratio, 
# de façon décroissante (du plus grand au plus petit)
...
```{r module5-question2-solution}
# Filtrer le registre des naissances pour l'année 1212 
# et enregistrer le résultat dans l'objet naissances_1212
naissances_1212 <- filter(naissances, An == 1212)

# Ajouter la variable Ratio comme le ratio
# entre la variable Taille et la variable Poids au carré
naissances_1212 <- mutate(naissances_1212, Ratio = Taille / Poids^2)

# Trier le fichier naissances_1212 selon le Ratio, 
# de façon décroissante (du plus grand au plus petit)
arrange(naissances,desc(Ratio))
...
A vous de jouer ! La première ligne du tableau final vous donnera le nom et surtout le village de naissance de cette petite fille pas tout à fait comme les autres...

```{r module5-question3,exercise=TRUE}
# Sélectionner les observations de genre féminin (F)
# et enregistrer le résultat dans l'objet naissances_filles


# Ajouter la variable Mana 
# Comme somme de la Force et de Spiritualité


# Triez par ordre décroissant selon le Mana


# fin de l'exercice
``` 
 
```{r module5-question3-solution}
# Sélectionner les observations de genre féminin (F)
# et enregistrer le résultat dans l'objet naissances_filles
naissances_filles <- filter(naissances,Genre=="F")

# Ajouter la variable Mana 
# Comme somme de la Force et de Spiritualité
naissances_filles <- mutate(naissances_filles,Mana = Force + Spirit)

# Triez par ordre décroissant selon le Mana
arrange(naissances_filles,desc(Mana))
...
**Fin du chapitre 8 >> reprenez la partie d'Icarius !!!** 