...
Au fait, vous n'avez pas remarqué une anomalie dans cette histoire ? A l'origine, la formule de calcul de l'âge se fonde sur le nombre de coeur à la naissance, et le nombre maximum de coeur que le statisien peut obtenir au maximum. Il vous a été indiqué ainsi, au début du chapitre 1, comme au début de ce chapitre 1(1), les indications suivantes :
...
Mais... dans le jeu vidéo, notre héros icaRius a déjà obtenu plus de 6 coeurs ! Il a dépassé la limite maximale autorisée `max_nb_coeur` ! Eh oui, icaRius n'est pas un statisien comme les autres. De même que la princesse Statia est née avec un Mana largement supérieur aux autres enfants de sa contrée, icaRius a pour lui de pouvoir acquérir bien plus de coeurs que ses concitoyens.

En l'occurrence, icaRius pourra avoir jusqu'à 4 fois le nombre initial de coeurs au début de l'aventure. 
